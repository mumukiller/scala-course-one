...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    if (isEmpty) Nil
      ...
      val tweetOfMostRetweeted = mostRetweeted
      new Cons(tweetOfMostRetweeted, remove(tweetOfMostRetweeted).descendingByRetweet)
    }
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    def maxRetweets(thisTween: Tweet, thatTweet: Tweet) : Tweet = if (thisTween.retweets > thatTweet.retweets) thisTween else thatTweet

    if (left.isEmpty && right.isEmpty) elem
    else if (left.isEmpty) maxRetweets(elem, right.mostRetweeted)
    else if (right.isEmpty) maxRetweets(elem, left.mostRetweeted)
    else maxRetweets(maxRetweets(left.mostRetweeted, right.mostRetweeted), elem)
  }


  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val childTweetSet = left.filterAcc(p, right.filterAcc(p, acc))
    if (p(elem)) childTweetSet.incl(elem) else childTweetSet
  }
